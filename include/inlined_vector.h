...
template<typename T, int InitialCapacity, bool CanExpand = false> 
...
public:
	...
	inlined_vector(const inlined_vector<T, InitialCapacity_, CanExpand_>& other):inlined_vector(other.begin(), other.size()){}
	...
	inline T& at(unsigned int i) { return const_cast<T&>(static_cast<const inlined_vector*>(this)->at(i)); }
...
protected:
	...
	inlined_vector(Iter begin_, unsigned int size){
		if (size > max_size()) {
	...
	inlined_vector(array_type&& array, unsigned int size):data_internal_(std::move(array)), size_(size){}

	inline T& element(unsigned int index){
...
template<typename T, int InitialCapacity> class inlined_vector<T, InitialCapacity, true>: public inlined_vector<T, InitialCapacity, false> {
public:
...
public:
	...
	template <int InitialCapacity_, bool CanExpand_> 
	inlined_vector(const inlined_vector<T, InitialCapacity_, CanExpand_>& other):inlined_vector(other.begin(), other.end(), other.size()){}

	template <class Container>
	inlined_vector(const Container& els):inlined_vector(els.begin(), els.end(), els.size()){}
	
	inlined_vector(std::initializer_list<T> els):inlined_vector(els.begin(), els.end(), els.size()){}

	inlined_vector(const inlined_vector& other):inlined_vector(other.begin(), other.end(), other.size()){}

	inlined_vector(inlined_vector&& other):base_t(std::move(other.data_internal_),other.size_),data_external_(std::move(other.data_external_)),inlined_(other.inlined_){
	...
	iterator begin() override final 				   { return inlined_ ? data_internal_.begin() : unwrap(data_external_.begin()); }
	const_iterator begin() const override final 	   { return inlined_ ? data_internal_.begin() : unwrap(data_external_.begin()); }
	reverse_iterator rend() override final 			   { return inlined_ ? data_internal_.rend()  : unwrap(data_external_.rend());  }
...
protected:
	// Helper constructor
	template <typename Iter>
	inlined_vector(Iter begin_, Iter end_, unsigned int size){
		size_ = size;
		if (size_ <= max_size()){
...
inline std::ostream& operator<<(std::ostream& out, const inlined_vector<T, N, false>& vector){
	...
	if (vector.empty()) out << "]";
		...
		for (auto it = vector.begin(); it != vector.end(); ++it){
			if (std::next(it) != vector.end()) out << *it << ", ";
			else out << *it << "]";
		}
	}
	return out;
...
inline std::ostream& operator<<(std::ostream& out, const inlined_vector<T, N, true>& vector){
	out << "inlined_vector ";
	if (vector.inlined_) out << "(inlined):  [";
	else out << "(external): [";
	if (vector.empty()) out << "]";
	else {
		for (auto it = vector.begin(); it != vector.end(); ++it){
